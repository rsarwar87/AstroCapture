...
class IVInspector {
 public:
  ...
  // In the inspector, we cannot rely on the ID stack, since calls to AddImages
  // will have a different stack than when will later display the image
  bool sDontUseIdStack = false;
  ...
  void Inspector_AddImage(const cv::Mat& image, const std::string& legend,
    ...
    std::string label =
        legend + "##" + std::to_string(s_Inspector_ImagesAndParams.size());
    auto id = sInspectorImageTextureCache.GetID(label, sDontUseIdStack);
    s_Inspector_ImagesAndParams.push_back(
        {id, label, image.clone(), params, zoomCenter, zoomRatio});

    // bump cache
    {
      auto& imageAndParams = s_Inspector_ImagesAndParams.back();
      sInspectorImageTextureCache.UpdateCache(id, imageAndParams.Image,
                                              &imageAndParams.Params, true);
    }
  }
  ...
  void priv_Inspector_ShowImagesListbox(float width) {
    ...
    if (ImGui::BeginListBox(
      ...
      for (size_t i = 0; i < s_Inspector_ImagesAndParams.size(); ++i) {
        ...
        auto id = sInspectorImageTextureCache.GetID(imageAndParams.Label,
                                                    sDontUseIdStack);
        auto& cacheImage =
            sInspectorImageTextureCache.GetCacheImageAndTexture(id);
...

        {
          ...
          if (ImGui::SmallButton("x")) idxSuppress = i;
          ImGui::PopStyleColor();
          ImGui::SameLine();
        }

        std::string id_selectable =
            imageAndParams.Label + "##_" + std::to_string(i);
        if (ImGui::Selectable(id_selectable.c_str(), is_selected, 0, itemSize))
          s_Inspector_CurrentIndex = i;

        float imageRatio = cacheImage.GlTexture->mImageSize.x /
                           cacheImage.GlTexture->mImageSize.y;
        ImVec2 image_tl(pos.x, pos.y + ImGui::GetTextLineHeight());
        ImVec2 image_br(pos.x + imageRatio * imageHeight,
                        image_tl.y + imageHeight);

        ImGui::GetWindowDrawList()->AddImage(cacheImage.GlTexture->mImTextureId,
                                             image_tl, image_br);
      ...
      if (idxSuppress >= 0)
        s_Inspector_ImagesAndParams.erase(s_Inspector_ImagesAndParams.begin() +
                                          (size_t)idxSuppress);
    }
  };
  ...
  void priv_Inspector_CleanImagesParams(const ImVec2& imageSize) {
      ...
      if (!i.WasSentToTextureCache) {
        if (i.InitialZoomRatio > 0.) {
          i.Params.ZoomPanMatrix = ZoomPanTransform::MakeZoomMatrix(
              i.InitialZoomCenter, i.InitialZoomRatio,
              i.Params.ImageDisplaySize);
        }
    ...
    if (s_Inspector_CurrentIndex < s_Inspector_ImagesAndParams.size()) {
      const auto& currentParams =
          s_Inspector_ImagesAndParams[s_Inspector_CurrentIndex].Params;
      for (auto& v : s_Inspector_ImagesAndParams) {
        ...
        v.Params.AddWatchedPixelOnDoubleClick =
            currentParams.AddWatchedPixelOnDoubleClick;
      }
    }
  };
  ...
  ImVec2 priv_Inspector_ImageSize(float listWidth, bool showOptionsColumn) {
    ...
    imageSize = ImVec2(winSize.x - listWidth - x_margin - image_options_width,
   ...
   void Inspector_Show(bool show_list = false) {
    ...
    //
    {
      ...
      if (s_Inspector_CurrentIndex < s_Inspector_ImagesAndParams.size()) {
        auto& imageAndParams =
            s_Inspector_ImagesAndParams[s_Inspector_CurrentIndex];
        Image(imageAndParams.Label, imageAndParams.Image,
